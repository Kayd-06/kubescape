...
func (yp *YamlPrinter) SetWriter(ctx context.Context, outputFile string) {
	if outputFile != "" {
		...
		if filepath.Ext(strings.TrimSpace(outputFile)) != printer.YamlOutputExt {
...
func (yp *YamlPrinter) convertToImageScanSummary(imageScanData []cautils.ImageScanData) (*imageprinter.ImageScanSummary, error) {
...
func printConfigurationsScanningYaml(opaSessionObj *cautils.OPASessionObj, imageScanData []cautils.ImageScanData, yp *YamlPrinter) error {
	...
	if imageScanData != nil {
		imageScanSummary, err := yp.convertToImageScanSummary(imageScanData)
		if err != nil {
...
func (p *YamlPrinter) CloseWriter() {
